...
var brushedQueue2 = d3.renderQueue(path_brushed)
  .rate(500)

pc.renderBrushed.queue = function() {
  if (isBrushed()) {
    brushedQueue2(__.brushed);
      console.log('Finally');
       k = __.brushed
      console.log(k);
      d3.csv('MedicalData.csv', function (data) {
    var color = function (d) {
      return colors(d.Diseases)
    }


    var parcoords = d3
      .parcoords()('#example-progressive2')
      .data(k)
      .hideAxis(['DiseasesGroups'])
      .color(color)
      .alpha(0.25)
      .composite('darken')
  ...
  g.append("svg:g")
    ...
    .append("svg:text")
      .attr({
        "text-anchor": "left",
        "y": 0,
        "transform": "translate(0,0) rotate(" +-65 + ")",
        "rotate": 0,
        "x": 0,
        "class": "label"
      })
